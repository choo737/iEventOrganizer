import React, { Component } from 'react';
import ReactDOM from 'react-dom';
import { Button, Checkbox, Form, Input, Radio, Select, TextArea, Dropdown, Menu, Segment, Image } from 'semantic-ui-react'

import { Events } from '../../api/events.js';
import EventImageModal from './SelectImageModal';

var EventInfo = {
  Title : "",
  Category : "",
  Division : "",
  Date : "",
  Time : "",
  Venue : "",
  Description : "",
  Organizer : ""
}

const eventCategoryOptions = [
  { key: '1', text: 'Dinner', value: 'dinner' },
  { key: '2', text: 'Outdoor', value: 'outdoor' },
  { key: '3', text: 'Sport', value: 'sport' },
  { key: '99', text: 'Others', value: 'others' },
]

const divisionOptions = [
  { key: '1', text: 'Business solution', value: 'bs' },
  { key: '2', text: 'Cloud solution', value: 'cs' },
  { key: '3', text: 'GSS', value: 'gss' },
  { key: '99', text: 'Others', value: 'others' },
]

const options = [
  { key: 1, text: 'Choice 1', value: 1 },
  { key: 2, text: 'Choice 2', value: 2 },
  { key: 3, text: 'Choice 3', value: 3 },
]

export default class EventEntry extends Component {
  constructor(){
    super();
    this.state = {
    	EventImage: 'data:image/png;base64,'
		};
		
    this.SaveEventImagePhoto = this.SaveEventImagePhoto.bind(this);
  }
    
  HandleInfoChange(event, data){
    var id = "";
    if(data != null)
      id = data.id;
    else
      id = event.target.id;

    switch(id){
      case 'title' :
      //   this.setState({VenueOutdoor : data.checked});
        EventInfo.Title = data.value;
        break;
      case 'category' :
      //   this.setState({VenueIndoor : data.checked});
        EventInfo.Category = data.value;
        break;
      case 'division' :
      //   this.setState({Weekdays : data.checked});
        EventInfo.Division = data.value;
        break;
      case 'date' :
      //   this.setState({Weekends : data.checked});
        EventInfo.Date = event.target.value;
        break;
       case 'time' :
      //   this.setState({Weekends : data.checked});
        EventInfo.Time = event.target.value;
        break;
      case 'venue' :
      //   this.setState({Weekends : data.checked});
        EventInfo.Venue = data.value;
        break;
      case 'description' :
      //   this.setState({Weekends : data.checked});
        EventInfo.Description = event.target.value;
        break;
    }
  }

  handleSubmit(e) {
    Events.insert({
      title: EventInfo.Title,
      date: EventInfo.Date,
      time: EventInfo.Time,
      venue: EventInfo.Venue,
      description: EventInfo.Description,
      division: EventInfo.Division,
      category: EventInfo.Category,
      image: this.state.EventImage
    });
  }

  SaveEventImagePhoto(photo){
    if(photo){
      this.setState({ EventImage : photo })
    }
  }

  render() {
    return (
      <Segment size="large">
      	<Form size='large'>
      	  <Image
      	    height={300} 
      	    width={300}
      	    label={<EventImageModal handleSave={this.SaveEventImagePhoto} headerMessage='Select an image for your event' />}
      	    src={this.state.EventImage}
      	  />
      	  <Button.Group floated='right' >
      	    <Button>Cancel</Button>
      	    <Button.Or />
      	    <Button positive onClick={()=>this.handleSubmit()} > Submit </Button>
      	  </Button.Group>
      	  <br /> <br /> <br />
      	  <Form.Field id="title" control={Input} label='Event title' placeholder='Event title'  onChange={this.HandleInfoChange.bind(this)} required />
      	  <Form.Group widths='equal'>
      	  	<Form.Field id="category" control={Select} label='Event category' options={eventCategoryOptions} placeholder='Event category' required
      	  	onChange={this.HandleInfoChange.bind(this)}/>
      	  	<Form.Field id="division" control={Select} label='Division' options={divisionOptions} placeholder='Division' required
      	  	onChange={this.HandleInfoChange.bind(this)}/>
      	  </Form.Group>
      	  <Form.Group widths='equal'>
      	    <Form.Field>
      	    	<label> Event date </label>
      	    	<input id="date" type="date" ref="eventDate" onChange={this.HandleInfoChange.bind(this)} required/> 
      	    </Form.Field>
      	    <Form.Field id="venue" control={Input} label='Venue' placeholder='Location...'  onChange={this.HandleInfoChange.bind(this)} />
      	  </Form.Group>
      	  <Form.Group widths='equal'>
      	    <Form.Field id="time" control={Input} label='Time' placeholder='From - to -'  onChange={this.HandleInfoChange.bind(this)} />
      	  </Form.Group>
      	  <Form.Field id="description" label='Event Description' control='textarea' rows='3' onChange={this.HandleInfoChange.bind(this)}/>
      	</Form>
      </Segment>
    );
  }
}
