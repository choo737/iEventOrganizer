...
import { Modal, Segment, Header, Label, Image, Button, Icon } from 'semantic-ui-react';
...
export default class UserImageModal extends Component{
  ...
  renderPhoto(){
    return(
      <div>
        {this.state.userImage? <img width={200} src={this.state.userImage} /> : <h4>Please Select an Image</h4>}
      </div>
    )
  }

  photoInputHandleChnage(){
  ...
  render(){
    return(
      <Modal trigger={<Label as='a' corner='left' icon='edit' onClick={this.handleOpen} />} 
        basic 
        size='small' 
      ...
      >
        ...
        <Modal.Content>
          <Segment.Group style={{paddingLeft: '140px'}}>
            <Button.Group>
              <Button>
                <input type='file' ref='photoInput' onChange={() => this.photoInputHandleChnage()} />                   
              </Button>                 
              <Button color='green' type="submit" onClick={(e)=>this.handleSubmit(e)}>
